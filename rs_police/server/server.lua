...
RegisterNetEvent("rs_police:Server:OpenStorage", function(key)
    ...
    local minGrade = storageData.MinGrade

    if minGrade == false then
        exports.vorp_inventory:AddPermissionTakeFromCustom(prefix, job, 0)
    else
        exports.vorp_inventory:AddPermissionTakeFromCustom(prefix, job, tonumber(minGrade))
    end
...
Citizen.CreateThread(function()
    Wait(200)
    VorpInv.RegisterUsableItem(ConfigFines.ItemName, function(data)
        local _source = data.source
        local Character = VORPcore.getUser(_source).getUsedCharacter
        if not Character then return end
        ...
        if canUseItem(job, grade) then
            TriggerClientEvent("rs_police:abrirFormulario", _source)
        else
            VORPcore.NotifyLeft(_source, ConfigFines.Textos.Notify.collect, ConfigFines.Textos.Notify.notpermisitem,"toasts_mp_generic", "toast_mp_customer_service", 4000, "COLOR_RED" )
        end
    end)
end)

function canUseItem(job, grade)
    if not ConfigFines.jobRequiredItem then
        return true
    end

    local jobCfg = ConfigFines.allowedJobsItem[job]
    if jobCfg then
        if jobCfg.minGrade == false or grade >= jobCfg.minGrade then
            return true
        end
    end
    return false
...
RegisterServerEvent("rs_police:guardarMulta")
AddEventHandler("rs_police:guardarMulta", function(data)
    local src = source
    ...
    if not targetUser then
        VORPcore.NotifyLeft(src, ConfigFines.Textos.Notify.collect, ConfigFines.Textos.Notify.notid, "toasts_mp_generic", "toast_mp_customer_service", 4000, "COLOR_RED")
        return
    end
    ...
    VORPcore.NotifyLeft(src, ConfigFines.Textos.Notify.collect, ConfigFines.Textos.Notify.correctfine, "toasts_mp_generic", "toast_mp_customer_service", 5000, "COLOR_GREEN")

    VORPcore.NotifyLeft(targetSrc, ConfigFines.Textos.Notify.collect, ConfigFines.Textos.Notify.recivefine .. " " .. string.format("%.2f", data.monto) .. "$", "toasts_mp_generic", "toast_mp_customer_service", 6000, "COLOR_BLUE")
end)

RegisterServerEvent("rs_police:abrirMenuPago")
AddEventHandler("rs_police:abrirMenuPago", function()
    local src = source
    ...
    exports.oxmysql:query('SELECT * FROM multas WHERE id_multado = ? AND pagada = 0', { id }, function(result)
        if #result > 0 then
            ...
            TriggerClientEvent("rs_police:mostrarMenuPago", src, elementos)
        else
            VORPcore.NotifyLeft(src, ConfigFines.Textos.Notify.collect, ConfigFines.Textos.Notify.notfine, "toasts_mp_generic", "toast_mp_customer_service", 5000, "COLOR_BLUE")
        end
    end)
end)

RegisterServerEvent("rs_police:pagarMulta")
AddEventHandler("rs_police:pagarMulta", function(multaId)
    local src = source
    local Character = VORPcore.getUser(src).getUsedCharacter
    ...
    exports.oxmysql:query('SELECT * FROM multas WHERE id = ?', { multaId }, function(result)
        if #result > 0 then
            ...
            if Character.money >= montoMulta then
                ...
                VORPcore.NotifyLeft(src, ConfigFines.Textos.Notify.collect, ConfigFines.Textos.Notify.corectpay, "toasts_mp_generic", "toast_mp_customer_service", 5000, "COLOR_GREEN")
            else
                VORPcore.NotifyLeft(src, ConfigFines.Textos.Notify.collect, ConfigFines.Textos.Notify.notmoney, "toasts_mp_generic", "toast_mp_customer_service", 4000, "COLOR_RED")
            end
        end
    end)
end)
...
function canCollectMoney(job, grade)
    if not ConfigFines.jobRequiredCollect then return true end
    local jobCfg = ConfigFines.allowedJobsCollect[job]
    if jobCfg then
        if jobCfg.minGrade == false or grade >= jobCfg.minGrade then
            return true
        end
...
RegisterNetEvent("rs_police:openCollecfines")
AddEventHandler("rs_police:openCollecfines", function()
    local _source = source
    local Character = VORPcore.getUser(_source).getUsedCharacter
    local job = Character.job

    if canOpenPanel(job) then
        ultimoSheriff = _source
        exports.oxmysql:query('SELECT * FROM multas', {}, function(result)
            for _, multa in ipairs(result) do
                multa.pagada = tonumber(multa.pagada)
            end
            TriggerClientEvent("rs_police:abrirPanelSheriff", _source, result)
        end)
    else
        VORPcore.NotifyLeft( _source, ConfigFines.Textos.Notify.collect, ConfigFines.Textos.Notify.notpermismenu, "toasts_mp_generic", "toast_mp_customer_service", 4000, "COLOR_RED" )
    end
end)

RegisterServerEvent("rs_police:solicitarMultas")
AddEventHandler("rs_police:solicitarMultas", function()
    local src = source
    local User = VORPcore.getUser(src)
    if not User then return end
    local Character = User.getUsedCharacter
    ...
    exports.oxmysql:query(query, {}, function(result)
        TriggerClientEvent("rs_police:recibirMultas", src, result)
    end)
end)

RegisterServerEvent("rs_police:recolectarMultas")
AddEventHandler("rs_police:recolectarMultas", function()
    local src = source
    local User = VORPcore.getUser(src)
    if not User then return end
    local Character = User.getUsedCharacter
    ...
    if not canCollectMoney(job, grade) then
        VORPcore.NotifyLeft(src, ConfigFines.Textos.Notify.collect, ConfigFines.Textos.Notify.notpermistocollect, "toasts_mp_generic", "toast_mp_customer_service", 4000, "COLOR_RED")
        return
    end
    ...
    exports.oxmysql:query(query, {}, function(result)
        if not result or #result == 0 then
            VORPcore.NotifyLeft(src, ConfigFines.Textos.Notify.collect, ConfigFines.Textos.Notify.notfinetocollect, "toasts_mp_generic", "toast_mp_customer_service", 5000, "COLOR_BLUE")
            return
        end
        ...
        if total > 0 then
            ...
            VORPcore.NotifyLeft(src, ConfigFines.Textos.Notify.collect, ConfigFines.Textos.Notify.received .. total .. "$ " .. ConfigFines.Textos.Notify.amount, "toasts_mp_generic", "toast_mp_customer_service", 5000, "COLOR_GREEN")
        end
    end)
end)

RegisterServerEvent("rs_police:eliminarMulta")
AddEventHandler("rs_police:eliminarMulta", function(id)
    local src = source
...
AddEventHandler("rs_police:eliminarMultaPendiente", function(id)
    ...
    if ConfigFines.jobRequiredCollect then
        local jobConfig = ConfigFines.allowedJobsCollect[job]
        if not jobConfig then return end

        if jobConfig.minGrade ~= false and grade < jobConfig.minGrade then
            VORPcore.NotifyLeft(src, ConfigFines.Textos.Notify.collect, ConfigFines.Textos.Notify.notrankpermis, "toasts_mp_generic", "toast_mp_customer_service", 4000, "COLOR_RED")
            return
        end
    end
