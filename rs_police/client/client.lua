...
RegisterNetEvent("rs_police:abrirFormulario")
AddEventHandler("rs_police:abrirFormulario", function()
    local ped = PlayerPedId()
...
RegisterNUICallback("registrarMulta", function(data, cb)
    ...
    TriggerServerEvent("rs_police:guardarMulta", data)
    cb({})
end)
...
RegisterNetEvent("rs_police:abrirPanelSheriff")
AddEventHandler("rs_police:abrirPanelSheriff", function()
    local ped = PlayerPedId()
    ...
    TriggerServerEvent("rs_police:solicitarMultas")
end)

RegisterNetEvent("rs_police:recibirMultas")
AddEventHandler("rs_police:recibirMultas", function(multas)
    SendNUIMessage({
...
RegisterNUICallback("recolectarMultas", function(_, cb)
    ...
    ClearPedTasks(ped)
    SetNuiFocus(false, false)
    TriggerServerEvent("rs_police:recolectarMultas")
    cb({})
end)

RegisterNUICallback("eliminarMulta", function(data, cb)
    TriggerServerEvent("rs_police:eliminarMulta", data.id)
    cb({})
end)
...
AddEventHandler("rs_police:multaPendienteEliminada", function(multas)
    ...
    TriggerEvent("vorp:NotifyLeft", ConfigFines.Textos.Notify.collect, ConfigFines.Textos.Notify.multaEliminada, "toasts_mp_generic", "toast_mp_customer_service", 4000, "COLOR_GREEN")
end)

RegisterNetEvent("rs_police:multaEliminada")
AddEventHandler("rs_police:multaEliminada", function()
    TriggerEvent("vorp:NotifyLeft", ConfigFines.Textos.Notify.collect, ConfigFines.Textos.Notify.multaEliminada, "toasts_mp_generic", "toast_mp_customer_service", 4000, "COLOR_GREEN" )
end)
...
Citizen.CreateThread(function()
    ...
    while true do
        ...
        for _, punto in ipairs(ConfigFines.puntosPago) do
            if #(playerCoords - punto) < 1.5 then
                ...
                if not mostrandoPago then
                    SendNUIMessage({
                        type = "showprompt",
                        text = ConfigFines.Textos.press
                    })
                    mostrandoPago = true
                end

                if IsControlJustReleased(0, ConfigFines.Prompt) then
                    TriggerServerEvent("rs_police:abrirMenuPago")
                end
                break
            end
        end
...
Citizen.CreateThread(function()
    if not ConfigFines.EnableBlips then return end

    for _, location in pairs(ConfigFines.BlipsFines) do
        local blipFines = N_0x554d9d53f696d002(1664425300, location.pos.x, location.pos.y, location.pos.z)
        SetBlipSprite(blipFines, location.sprite, 1)
        SetBlipScale(blipFines, 0.2)
        Citizen.InvokeNative(0x9CB1A1623062F402, blipFines, location.name)
        table.insert(Blips, blipFines)
    end
end)

Citizen.CreateThread(function()
    if not ConfigFines.EnableNPCs then return end

    for _, coords in pairs(ConfigFines.NPC.coords) do
        TriggerEvent("rs_police:CreateNPC", coords)
    end
end)

RegisterNetEvent("rs_police:CreateNPC")
AddEventHandler("rs_police:CreateNPC", function(zone)
    if not zone then return end

    local model = GetHashKey(ConfigFines.NPC.model)
    RequestModel(model)
...
AddEventHandler('onResourceStop', function(resourceName)
    ...
    for _, blipFines in pairs(Blips) do
        RemoveBlip(blipFines)
    end
