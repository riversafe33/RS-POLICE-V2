...
function OpenPoliceMenu()
    ...
    local elements = {
        ...
        { label = ConfigFines.Textos.menuCollectFines,  value = 'viewfines' },
    }
    Menu.Open('default', GetCurrentResourceName(), 'menuapi',
        ...
        function(data, menu)
            ...
            elseif (data.current.value == 'idmenu') then
                OpenIDMenu()
            elseif (data.current.value == 'viewfines') then
                TriggerServerEvent("rs_police:openCollecfines")    
                menu.close()
            end
        end,
        function(data, menu)
...
RegisterNetEvent("rs_police:mostrarMenuPago")
AddEventHandler("rs_police:mostrarMenuPago", function(multas)
    local menuElements = {}

    for _, multa in ipairs(multas) do
        table.insert(menuElements, {
            label = ConfigFines.Textos.menuamount .. multa.label,
            value = multa.value,
            desc = ConfigFines.Textos.menureason,
        })
    end

    Menu.Open("default", GetCurrentResourceName(), "menuPagoMultas", {
        title = ConfigFines.Textos.menutitle,
        align = ConfigFines.Align,
        elements = menuElements
    }, function(data, menu)
        TriggerServerEvent("rs_police:pagarMulta", data.current.value)
        Menu.CloseAll()
    end, function(data, menu)
        Menu.CloseAll()
    end)
